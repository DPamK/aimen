...
def current_work():
    ...
    cursor.execute("""
        ...
        JOIN requirements r ON f.requirement_id = r.id
        WHERE t.status IN ('write_test', 'red', 'green', 'refactor')
...
def overview():
    ...
    # Requirements stats
    cursor.execute("SELECT status, COUNT(*) AS cnt FROM requirements GROUP BY status")
    req_stats = {row["status"]: row["cnt"] for row in cursor.fetchall()}
    ...
    return {
        ...
        "summary": (
            f"需求 {sum(req_stats.values()) if req_stats else 0} 个，"
